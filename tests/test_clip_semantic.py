...
class ClipCompetitionConfigTests(unittest.TestCase):
    def test_new_competition_config_uses_trained_zero_shot_route(self) -> None:
        ...
        self.assertEqual(
            config["zero_shot"]["backend"], "moeclip_official"
        )
        self.assertEqual(
            config["zero_shot"]["model"]["pretrained"], "openai"
        )
        self.assertEqual(
            config["zero_shot"]["training"]["optimizer"], "adam"
        )
        self.assertNotIn("synthesis", config["zero_shot"])
        self.assertEqual(config["zero_shot"]["model"]["levels"], [6, 12, 18, 24])
        self.assertEqual(config["zero_shot"]["model"]["moe_rank"], 8)
